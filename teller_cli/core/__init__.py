...
def get_world(token: str, url: str, world_name: str):
    response = httpx.get(
        url=f"{url}/worlds/find/{world_name}", headers={"X-Space-App-Key": token}
    )

    print(f"> Looking for world {world_name}")
    ...
    if world_id:
        print(f"> Found {world_name}:{world_id}")
...
def update_world_image(folder_path: str, world_id: str, url: str, token: str):
    ...
    response = httpx.patch(
        ...
        headers={"X-Space-App-Key": token, "Content-Type": "application/json"},
        json={"image": encode_image(icon_path)},
    )
...
def create_world(token: str, url: str, world_name: str, world_size: int):
    print(f"> Creating new world {world_name}")

    response = httpx.post(
        url=f"{url}/worlds/",
        headers={"X-Space-App-Key": token, "Content-Type": "application/json"},
        json={"name": world_name, "size": world_size},
    )
    ...
    print(f"> Succesfully created world {world_name}:{world_id}")
...
def chunk_and_upload(file_path: str, world_id: str, url: str, token: str):
    ...
    session_response = client.post(
        url=f"{url}/snapshots/upload",
        params={"file_name": file_name, "world_id": world_id},
    ...
    print(f"> Upload session {session_id} intiated")
    ...
    file_table = Table(
        title="[bold]ChunkVault-[bold purple]Lite [white]Upload", min_width=200
    )
        ...
        with open(file_path, "rb") as f:
            while finished is False:
                ...
                headers = {
                    ...
                    "content-range": f"bytes {current_chunk}-{current_chunk+chunk_size}/{total_size}",
                }

                params = {"file_name": file_name}

                looped_response = client.post(
                    url=f"{url}/snapshots/upload/{session_id}",
                    params=params,
                    headers=headers,
                    files={"file": chunk},
                )
